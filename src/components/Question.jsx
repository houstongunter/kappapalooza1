...
const Question = () => {
  ...
  return (
    <motion.div ref={ref} className="h-[93vh] bg-gradient-to-b from-kappa-blue to-kappa-bluetwo shadow-none">
      <div className="flex flex-col justify-center items-center h-full text-3xl text-white space-y-8">
        ...
        <motion.p className="text-8xl" initial="hidden" animate={isInView ? "visible" : "hidden"} variants={popInAnimation8xl}>
          Riverside&apos;s{" "}
          <span className="text-kappa-orangeyellow">BIGGEST</span>
        </motion.p>
        <motion.p initial="hidden" animate={isInView ? "visible" : "hidden"} variants={popInAnimation9xl} className="text-9xl">
